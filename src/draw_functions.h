...
#define Large 6 // Размер большой иконки погоды. Меняется изменением числа
#define Small 3 // Размер маленьких иконок погоды
typedef struct {
  int x;
  int y;
  int h;
...
void draw_weather()
{
  ...
  int forecastY = 80; //80 Расположение нижних блоков прогноза погоды на день
  int forecastW = 42; //42

  for (int i = 0; i < MAX_READINGS; i++)
  {
    draw_forecast(i * forecastW, forecastY, forecastW, 46, i + 1); // First  3hr forecast box 46
  }
  draw_battery(50, 14);//70,20
  display_update();
}
...
void draw_forecast(const int x, const int y, const int w, const int h, const int index)
{
  ...
  draw_string(x + w / 2, y - 12, WxForecast[index].Period.substring(11, 16), CENTER);//11,16
  ...
  display.drawLine(x + w, y - 15, x + w, y - 15 + h, FG_COLOR);
}

void draw_heading_section()
{
  display.setFont(&DEFALUT_FONT);
  draw_string(0, 10, time_str, LEFT);
  draw_string(115, 10, date_str, LEFT);
  display.drawLine(0, 12, SCREEN_WIDTH, 12, FG_COLOR);
}

void draw_main_weather_section()
{
  draw_wx_icon(130, 40, 0, LargeIcon); //162,49 расположение основной иконки погоды 
  display.setFont(&CITY_FONT);
  draw_string(0, 26, "Минск", LEFT);//Config::getCity() 0,26 расположение Названия города
  draw_temp(0, 53, 2, WxConditions[0].Temperature, &TEMP_FONT); //0, 53 Расположение температуры
  if (temperature>=0 && temperature < 100) {
    draw_temp(40, 53, 2, temperature, &CITY_FONT); //0, 53 Расположение температуры
  }
  display.setFont(&WEATHER_FONT); 
  draw_pressure_trend(65, 16, WxConditions[0].Pressure, WxConditions[0].Trend); //40,42 расположение давления
  draw_humidity(65, 36, WxConditions[0].Humidity); //90,50 расположение влажности
  draw_wind(190, 36,  WxConditions[0].Winddir + 180, WxConditions[0].Windspeed); //222, 44 Расположение по экрану иконы направления ветра 180
  display.drawLine(0, 0, SCREEN_WIDTH, 0, FG_COLOR);
}
...
#define Cradius 9 // размер иконки направления ветра
  ...
  display.setFont(&WEATHER_FONT);
  draw_string(x, y + Cradius + 13, wind_deg_to_direction(angle + 180), CENTER); // тут меняется растояние от цифр до окружности
  draw_string(x, y - Cradius - 5, String(windspeed, 1) + (Config::isMetric() ? " м/с" : " mph"), CENTER); // и тут
}
//#########################################################################################
String wind_deg_to_direction(float winddirection)
{
...
void draw_pressure_trend(int x, int y, float pressure, String slope)
{
  ...
  draw_string(x, y + 8, String(pressureHgMm, 0) + "мм", LEFT);
  x = x + 43;
  y = y + 3;
  if (slope == "+")
  {
